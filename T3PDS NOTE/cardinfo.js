...
function copyCommand() {
    ...
    const command = `coap -O65001,0 -Y coaps+tcp://${name || '[$K2V4_IP]'}/api-v1/healthcheck/boot | grep "card_info"`;
...
function false1() {
    const hobbyText = `nvme list | grep -c nvme`; // Fixed text
    const button = event.target;
...
function false2() {
    const hobbyText = `lspci -vvv | grep -c AL10`; // Fixed text
    const button = event.target;
...
function false3() {
    const hobbyText = `lspci -vvv | grep -c Amazon`; // Fixed text
    const button = event.target;
...
function false4() {
    const hobbyText = `lspci -vt`; // Fixed text
    const button = event.target;
